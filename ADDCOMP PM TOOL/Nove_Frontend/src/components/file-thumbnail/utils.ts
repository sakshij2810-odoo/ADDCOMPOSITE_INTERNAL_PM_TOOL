...
export const iconUrl = (icon: string) => `${CONFIG.assetsDir}/assets/icons/files/${icon}.svg`;
...
export function fileThumb(fileUrl: string) {
  let thumb;
  const type = isTypeFolder(fileUrl) ? "folder" :
    console.log("fileThumb type ===>", fileUrl)
...
export function isTypeFolder(fileUrl: string): boolean {
  return fileUrl.split('.').length > 1 ? false : true
}
