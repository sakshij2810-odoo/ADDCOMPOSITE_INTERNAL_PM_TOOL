...
import { IChatConversation } from "../types/IChatTypes";
...
export function getNavItem({ currentUserId, conversation }: Props) {
  ...
  const participantsInConversation = (participants || []).filter(
    (participant) => participant.user_uuid !== currentUserId
  );
  ...
  const displayName = participantsInConversation.map((participant) => participant.user_name).join(', ');
