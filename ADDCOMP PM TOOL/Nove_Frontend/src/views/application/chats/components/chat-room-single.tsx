...
export function ChatRoomSingle({ participant }: Props) {
  ...
  const renderInfo = (
    <Stack alignItems="center" sx={{ py: 5 }}>
      ...
      />
      <Typography variant="subtitle1">{participant?.user_name}</Typography>
  ...
  const renderContact = (
    <Stack spacing={2} sx={{ px: 2, py: 2.5 }}>
      {[
        { icon: 'mingcute:location-fill', value: "" },
        { icon: 'solar:phone-bold', value: participant?.phone_number },
        { icon: 'fluent:mail-24-filled', value: participant?.user_email },
      ].map((item) => (
        <Stack
          key={item.icon}
          spacing={1}
