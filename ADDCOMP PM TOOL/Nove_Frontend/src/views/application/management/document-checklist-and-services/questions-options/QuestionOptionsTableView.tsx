...
import type { IQuestion, IQuestionnaire, IQuestionOption, IStoreState } from 'src/redux';
...
const QuestionOptionsTableView = () => {
  ...
  const fetchList = () => {
    dispatch(fetchRecordCountAsync("latest_questions_options"))
    dispatch(
    ...
    masterColumns: [
      {
        ...
        onRowCellRender: (value, row: IQuestionOption) => { }
        // <StandardTableActions onEditClick={() => router.push(`${main_app_routes.app.questionnaire.questionOptions}/manage/${row.questions_options_uuid}`)} />
        ,
      },
      {
        key: "status",
      ...
      {
        key: "question_option",
        headerName: "Questionnaire Option",
        fieldName: "question_option",
  ...
  const hanndeCreateNewClick = () => {
    // router.push(`${main_app_routes.app.questionnaire.questionOptions}/manage`)
  }

  return (
    <DashboardContent metaTitle='Question Options'>
      <CustomBreadcrumbs
        heading="Question Options"
        links={[
          // { name: 'Question Options', href: main_app_routes.app.questionnaire.questionOptions },
          { name: 'List' },
        ]}
        action={
          <Button
...
export default QuestionOptionsTableView