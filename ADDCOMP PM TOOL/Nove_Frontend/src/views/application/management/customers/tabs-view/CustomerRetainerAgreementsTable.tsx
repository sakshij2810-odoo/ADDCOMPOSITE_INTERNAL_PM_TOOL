...
import { CreateNewRetainerAgreementDailog } from 'src/views/application/leads/private-leads/tabs-view/RetainerAgreement/dialogs/CreateNewRetainerAgreementDailog';
import { PreviewRetainerAgreementDialog } from 'src/views/application/leads/private-leads/tabs-view/RetainerAgreement/dialogs/PreviewRetainerAgreementDialog';
import { DocumentStatusRenderer, SignDocumentButton } from 'src/views/application/leads/documents/components';
import { useCustomerContext } from '../provider';

export const CustomerRetainerAgreementsTable = () => {
    ...
    const fetchList = () => {
        if (!uuid) return
        dispatch(fetchRetainerAgrewementWithArgsAsync({ module: "CUSTOMER", uuid }));
    };
    ...
    return (
        <MuiStandardCard title='Retainer Agreements'
            rightHeading={<Button variant='contained' onClick={() => setOpenUpsertDocumentDialog({
                ...defaultRetainerAgreement,
                client_name: (`${customerInfo.customer_first_name} ${customerInfo.customer_last_name || ""}`).trim(),
                // service_type: customerInfo.service_type,
                // service_sub_type: customerInfo.service_sub_type,
                module_uuid: customerInfo.customer_fact_uuid as string,
                module_name: "CUSTOMER",
                client_email: customerInfo.customer_email,
                client_contact_number: customerInfo.customer_phone_number
