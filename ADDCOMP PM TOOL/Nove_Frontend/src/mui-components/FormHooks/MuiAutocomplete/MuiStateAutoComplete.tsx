...
export interface IMuiStateAutoCompleteProps {
  label: string;
  country?: string;
  placeholder?: string;
  hiddenLabel?: boolean;
...
export const MuiStateAutoComplete: React.FC<IMuiStateAutoCompleteProps> = (
  props,
) => {
  const { label, value, onSelect, disabled, error, placeholder, country } = props;
    ...
    try {
      const res = await axios_base_api.get(server_base_endpoints.general.get_country_state, {
        params: {
          ...
          ...(searchableValue?.length > 0 && { columns: 'state_name', value }),
        }
      });
      const finalData: ICountryState[] = res.data.data;
      setOptions(finalData.map((service) => service.state_name));
    } catch (error) {
