...
const d3HierarchyLayout: LayoutAlgorithm = async (nodes: any, edges: any, options: any) => {
  ...
  const root: any = layout(hierarchy);
  ...
  const nextNodes = nodes.map((node: any) => {
    ...
    const position: any = getPosition(x, y, options.direction);
