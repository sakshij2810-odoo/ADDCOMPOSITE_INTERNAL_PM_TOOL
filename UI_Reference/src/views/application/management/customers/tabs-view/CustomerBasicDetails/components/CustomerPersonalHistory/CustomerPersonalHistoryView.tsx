...
import { ICustomerPersonalHistory, IEducationProfile } from 'src/redux/child-reducers'
import { CustomerPersonalHistoryRow } from './CustomerPersonalHistoryRow'
import { produce } from 'immer'
import { Button, Table, TableBody, TableCell, TableHead, TableRow } from '@mui/material'
import { CustomerPersonalHistoryDailog } from './dialogs/CustomerPersonalHistoryDailog'
...
interface ICustomerPersonalHistoryViewProps {
    title: string | React.ReactNode
    data: ICustomerPersonalHistory[]
    onChange: (edu: ICustomerPersonalHistory[]) => void

}

export const CustomerPersonalHistoryView: React.FC<ICustomerPersonalHistoryViewProps> = ({ title, data: educationDetails, onChange }) => {
    const [openDetailsPanel, setOpenDetailsPanel] = useState(true)
    const [openDeleteDialog, setOpenDeleteDialog] = useState<{ index: number, data: ICustomerPersonalHistory } | null>(null)
    const [opneAddNewDialog, setOpneAddNewDialog] = useState<{ index: number | null, data: ICustomerPersonalHistory | null } | null>(null)



    const handlEducationSaveSuccess = (idx: number | null, eduObj: ICustomerPersonalHistory) => {
                        ...
                        <Table   >
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell>Company/Employer Name</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody >
                                {educationDetails.map((education, e_idx) => {
                                    return (
                                        <CustomerPersonalHistoryRow key={e_idx}
                                            data={education}
