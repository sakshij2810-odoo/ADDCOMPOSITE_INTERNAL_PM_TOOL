...
import { clearQuestionsWithAnswersListStateSync, defaultAnswer, fetchMultipleQuestionsAsync, fetchMultipleQuestionsOfQuestionnaireAsync, fetchMultipleQuestionsWithAnswersAsync, IAnswer, IQuestion, IQuestionnaireQuestionAnswer, IStoreState, upsertSingleAnswerWithCallbackAsync, useAppDispatch, useAppSelector } from 'src/redux';
...
export const DocumentChecklistTable: React.FC<IDocumentChecklistTableProps> = ({ questionnaire_uuid, service_uuid }) => {
    ...
    const {
        ...
        loading
    } = useAppSelector((storeState: IStoreState) => storeState.management.questionnaire.questions_with_answers_list);
...
function DocumentChecklistRow({ row, service_uuid }: Props) {
    ...
    const {
        ...
        initialValues: row.answer ? row.answer : {
            ...defaultAnswer,
    ...
    useEffect(() => {
        setValues(row.answer ? row.answer : {
            ...defaultAnswer,
            questionnaire_uuid: row.question.questionnaire_uuid,
            questionnaire_name: row.question.questionnaire_name,
            questions_uuid: row.question.questions_uuid,
            ...
            <TableRow hover tabIndex={-1} >
                ...
                <TableCell >
                    <MuiFormFields.MuiDynamicField
                        ...
                        options={row.options.map((option) => ({ label: option.question_option as string, value: option.questions_options_uuid as string }))}
