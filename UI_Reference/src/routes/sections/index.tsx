...
export function Router() {
  return useRoutes([
    {
      path: '/',
      element: CONFIG.auth.skip ? <>{appLayoutContent}</> : <AuthGuard>{appLayoutContent}</AuthGuard>,
      children: [
        { element: <IndexPage />, index: true },
        ...leadsRoutes
      ].concat(configurationRoutes)
        .concat(managementRoutes).concat(componentsRoute)
        .concat(filemanagerRoute).concat(settingsRoutes)
        .concat(dataManagementRoutes).concat(chatsRoutes).concat(tasksRoutes),
    },
