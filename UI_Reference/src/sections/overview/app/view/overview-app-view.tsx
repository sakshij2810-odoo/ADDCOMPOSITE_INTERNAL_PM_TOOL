...
import { fetchGeneralRecordCountAsync, fetchGeneralRecordCountAsyncV2, fetchLatestLeadsAsync } from '../services/app-services.services';
import { IPrivateLead } from 'src/redux';
import { LatestLeadsDashboard } from '../LatestLeads.dashboard';

// ----------------------------------------------------------------------

export function OverviewAppView() {
  const { user } = useMockedUser();
  const { user: { first_name, last_name } } = useAuthContext();
  const theme = useTheme();
  const [widgetCountResult, setWidgetCountResult] = useState<{
  ...
  useEffect(() => {
    fetchGeneralRecordCountAsyncV2().then((result) => {
      setWidgetCountResult(result)
    })
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={4}>
          <AppWidgetSummary
            ...
            percent={widgetCountResult.leads.growth}
            total={widgetCountResult.leads.count}
            chart={{
        ...
        <Grid xs={12} md={4}>
          <AppWidgetSummary
            ...
            percent={widgetCountResult.customers.growth}
            total={widgetCountResult.customers.count}
            chart={{
        ...
        <Grid xs={12} md={4}>
          <AppWidgetSummary
            ...
            percent={widgetCountResult.users.growth}
            total={widgetCountResult.users.count}
            chart={{
        ...
        <Grid xs={12} md={4}>
          <AppWidgetSummary
            ...
            percent={widgetCountResult.pending_invoices.growth}
            total={widgetCountResult.pending_invoices.count}
            chart={{
              categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug'],
              series: [15, 18, 12, 51, 68, 11, 39, 37],
            }}
        ...
        <Grid xs={12} md={4}>
          <AppWidgetSummary
            ...
            percent={widgetCountResult.paid_invoices.growth}
            total={widgetCountResult.paid_invoices.count}
            chart={{
              colors: [theme.vars.palette.info.main],
              categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug'],
              series: [20, 41, 63, 33, 28, 35, 50, 46],
        ...
        <Grid xs={12} md={4}>
          <AppWidgetSummary
            title="Total Partially Paid Invoices"
            percent={widgetCountResult.partially_paid_invoices.growth}
            total={widgetCountResult.partially_paid_invoices.count}
            chart={{
              colors: [theme.vars.palette.error.main],
              categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug'],
              series: [18, 19, 31, 8, 16, 37, 12, 33],
        ...
        <Grid xs={12} lg={12}>
          <LatestLeadsDashboard />
          {/* <AppNewInvoice
            title="New invoice"
            tableData={latestLeads}
            headLabel={[
